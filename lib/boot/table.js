...
Table.prototype.render = function(list, od) {
  ...
  var denter = datum.enter()
    ...
    .on('click', function(data) {
      if (!this.transitive) {
        ...
        var profileResponse = new OtpProfiler.models.OtpProfileResponse({ 'options' : data });
        var TransitiveLoader = new OtpProfiler.transitive.TransitiveLoader(profileResponse, window.CONFIG.OTP_API_URL, function(transiveData) {
  ...
  denter.append('div')
    .attr('class', 'times col-lg-1 col-md-1 col-sm-1')
    .html(function(d) {
      return '<i class="fa fa-heart"></i> ' + metersToMiles(d.segments.reduce(function (mi, d) {
