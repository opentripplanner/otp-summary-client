...
Table.prototype.render = function(list) {
  ...
  denter.append('div')
    ...
    .html(function(d) {
      if (d.stats.avg === d.stats.max) return secondsToMinutes(d.stats.avg) + ' min';
      else return secondsToMinutes(d.stats.avg) + '-' + secondsToMinutes(d.stats.max) + ' min';
    });
  ...
  denter.append('div')
    ...
    .selectAll('.segment')
    .data(function(d) {
      var paths = d.segments.slice();
      if (d.stats.max > d.stats.avg) {
        paths.push({
          routeShortName: 'lightgrey',
          stats: {
            avg: d.stats.max - d.stats.avg
          }
        });
      }
      return paths;
    ...
    .style('width', function(d) {
      return d.stats.avg / maxTime * 100 + '%';
    })
  ...
  details.append('div')
    ...
    .html(function (d) {
      return d.segments.reduce(function (s, c) {
        return s + (c.routeLongName || c.routeShortName || c.route) + '<br>';
      }, '');
    });
