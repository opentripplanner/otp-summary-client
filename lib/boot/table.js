...
Table.prototype.render = function(list) {
  ...
  var maxTime = d3.max(list, function(d) {
    return d.stats.max + d.segments.reduce(function (s) {
      return s.walkDist / WALK_SPEED;
    }) + d.finalWalkDist / WALK_SPEED;
  });
  ...
  var tempScale = d3.scale.pow().range([ 10, 300 ]);
    ...
    .data(function(d) {
      ...
      d.segments.forEach(function(segment) {
        paths.push({
          ...
          stats: {
            avg: segment.walkDist / WALK_SPEED
          }
        });
        paths.push(segment);
      });
      ...
      paths.push({
        routeShortName: 'lightgrey',
        stats: {
          avg: d.finalWalkDist / WALK_SPEED
        }
      });
