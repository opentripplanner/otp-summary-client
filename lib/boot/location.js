...
var changeLocations = module.exports.changeLocations = function(from, to) {
  ...
  llFromString(from, function(fll) {
    llFromString(to, function(tll) {
      $('.from-display-value').html(fll.address + ', ' + fll.city + ', ' +
        fll.state);
      $('.to-display-value').html(tll.address + ', ' + tll.city + ', ' +
        tll.state);

      jsonp(config.OTP_API_URL + '?from=' + fll.lat + ',' + fll.lon +
...
$('a.set-location').on('click', function(event) {
  ...
  window.history.pushState({}, null, '/#results?from=' + encodeURIComponent(
